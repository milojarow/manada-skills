...
| Verifying a control (test/check) an implementer delivers alongside its own fix — why breaking the code isn't enough without a positive-control pass too, the 3 shapes of a hollow control (import-only, self-fabricated input, unregistered), and why a control whose rule was revoked gets re-pointed at the new boundary instead of deleted | [reference/control-verification.md](reference/control-verification.md) |
| Contract drift between two sessions working opposite sides of the same API — why a closed enum against server-controlled values can blank an entire screen on an "additive" server change, the rule for when to tolerate an unknown value by name vs. keep rejecting it, and how to audit a codebase for the blast radius | [reference/contract-drift.md](reference/contract-drift.md) |
