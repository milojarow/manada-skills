...
| Onboarding a NEW adversarial-reviewer agent/model — why a "found nothing" over real code is not evidence without a positive control first, the planted-defect method, and how the measurement harness itself can be the thing that's broken | [reference/reviewer-calibration.md](reference/reviewer-calibration.md) |
